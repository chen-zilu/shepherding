...
def PDE_simulation_2D(params, gamma, delta, behav_type, DirectoryName):
    ...
    for step in range(1, numSteps+1):
        ...
        def compute_rhs(uH_sp_hat, uT_sp_hat):
            ...
            rhs_uH_x = SRR * np.fft.fft2(uH_sp * grad_uH_x) + SRR * np.fft.fft2(uT_sp * grad_uH_x) - np.fft.fft2(v2 * uH_sp * grad_uH_x) - np.fft.fft2(v1_x * uH_sp * uT_sp)
            rhs_uH_y = SRR * np.fft.fft2(uH_sp * grad_uH_y) + SRR * np.fft.fft2(uT_sp * grad_uH_y) - np.fft.fft2(v2 * uH_sp * grad_uH_y) - np.fft.fft2(v1_y * uH_sp * uT_sp)
