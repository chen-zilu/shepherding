...
def compute_laplacian(U, dx, dy):
    ...
    # 处理边界 (np.roll 会循环绕回，所以必须把边界设为0或不计算)
    # 简单做法：再次强制边界为0，或者你的 set_boundary_zero 会处理
    # 你的 set_boundary_zero 是处理 rhs 的，所以这里算出来会有边界误差，
    # 但由于你最后都会 set_boundary_zero，所以没关系。
...
def PDE_simulation_2D(params, gamma, delta, behav_type, DirectoryName):
    ...
    # 左下区域 (uH)
    x1_H, x2_H = Nx * 5 // 16, Nx * 7 // 16
    y1_H, y2_H = Nx * 7 // 16, Nx * 9 // 16
    ...
    for step in range(1, numSteps + 1):
        ...
        def compute_rhs_nofft(uH_sp, uT_sp):
            ...
            for idx, sp in enumerate(scout_pos):
                ...
                sc_herder_v_weights = np.exp(-100 * sc_uT)
            ...
            rhs_uH_x = (
                ...
                - kh * (v1_x * uH_sp * uT_sp) * 50 /(1+10*uH_sp)
                - scout_vX * uH_sp
            )
            rhs_uH_y = (
                # + D * grad_uH_y
                ...
                - kh * (v1_y * uH_sp * uT_sp) * 50 /(1+10*uH_sp)
                - scout_vY * uH_sp
            )
            # 4. uT 的两个方向通量
            rhs_uT_x = (
